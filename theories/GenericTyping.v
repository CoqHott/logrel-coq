...
Section GenericTyping.
  ...
  Class RedTermProperties :=
  {
    ...
    redtm_fst_beta {Γ A B B' a b} :
      [Γ |- A] ->
      [Γ ,, A |- B] ->
      [Γ |- a : A] ->
      [Γ |- b : B[a..]] ->
      B' =o B ->
      [Γ |- tFst (tPair B' a b) ⇒* a : A] ;
    redtm_fst {Γ A B p p'} :
      [Γ |- p ⇒* p' : tSig A B] ->
      [Γ |- tFst p ⇒* tFst p' : A] ;
    redtm_snd_beta {Γ A B B' a b} :
      [Γ |- A] ->
      [Γ ,, A |- B] ->
      [Γ |- a : A] ->
      [Γ |- b : B[a..]] ->
      B' =o B ->
      [Γ |- tSnd (tPair B' a b) ⇒* b : B[(tFst (tPair B' a b))..]] ;
    redtm_snd {Γ A B p p'} :
      [Γ |- p ⇒* p' : tSig A B] ->
      [Γ |- tSnd p ⇒* tSnd p' : B[(tFst p)..]] ;
...
Section GenericConsequences.
  ...
  Proof.
    intros.
    eapply ty_conv.
    2: now eapply convty_simple_arr.
    eapply ty_lam ; tea.
    2: easy.
    now eapply ty_var0.
  Qed.
  ...
  Lemma convtm_id {Γ A} : 
    [Γ |- A] ->
    [Γ |- A ≅ A] ->
    [Γ |- idterm ≅ idterm : arr A A].
  Proof.
    intros.
    assert [|- Γ,, A] by gen_typing.
    assert [Γ,, A |-[ ta ] A⟨↑⟩] by (renToWk ; now eapply wft_wk).
  ...
  Lemma redtm_comp_beta {Γ A B C f g a} :
    ...
    [Γ |- g : arr B C] ->
    [Γ |- a : A] ->
    [Γ |- tApp (comp g f) a ⇒* tApp g (tApp f a) : C].
  Proof.
    intros hA hB hC hf hg ha.
    eapply redtm_meta_conv.
    1: eapply redtm_beta ; cycle -2 ; tea.
    + easy. 
